    ...
    return (
        <button
            ...
            className={[
                ...
                "text-jse-principal shadow-lg shadow-black/10 backdrop-blur-md",
                "active:scale-95",
                active ? "text-jse-accent" : "",
            ].join(" ")}
        >
            {children}
        </button>
...
export default function RestaurantDetail() {
    ...
    const restaurantEstNumerique =
        restaurantId !== null &&
        restaurantId !== undefined &&
        String(restaurantId).trim() !== "" &&
        Number.isInteger(Number(restaurantId));

    useEffect(() => {
        if (restaurantEstNumerique) {
            setFavori(Boolean(favoriServeur));
            return undefined;
        }

        const synchroniser = () => setFavori(estFavori(restaurantId));
    ...
    }, [restaurantEstNumerique, restaurantId, favoriServeur]);

    const imageRestaurant =
        imagesRestaurants[
            Math.abs(Number(restaurantId || 1) - 1) % imagesRestaurants.length
        ];
    ...
    const produitsFiltres = useMemo(() => {
        ...
        return produits.filter((produit) => {
            const matchCat =
                categorie === "Toutes" || produit.categorieId === categorie;
    ...
    const partager = async () => {
        ...
        if (navigator.share) {
            await navigator.share({
                title: restaurant?.nom || "JSE Express",
                text: `Découvrez ${restaurant?.nom || "ce restaurant"} sur JSE Express.`,
                url,
            });
            return;
        }
    ...
    const basculerRestaurantFavori = () => {
        if (restaurantEstNumerique) {
            const id = Number(restaurantId);
            const prochainEtat = !favori;

            setFavori(prochainEtat);

            router.visit(`/favoris/${id}`, {
                method: prochainEtat ? "post" : "delete",
                preserveScroll: true,
                preserveState: true,
                onError: () => setFavori(!prochainEtat),
            });

            return;
        }

        const nouveauxFavoris = basculerFavori({
            ...restaurant,
            image: imageRestaurant,
        });

        setFavori(
            nouveauxFavoris.some(
            ...
            <div className="mx-auto flex min-h-screen w-full max-w-[1440px] lg:px-8">
                ...
                <div className="min-w-0 flex-1">
                    <div className="relative mx-auto min-h-screen w-full max-w-[760px] overflow-hidden bg-jse-fond shadow-none lg:my-6 lg:rounded-[32px] lg:shadow-2xl">
                            ...
                            <div className="absolute inset-x-0 top-0 flex items-center justify-between px-5 pt-6 sm:px-7 sm:pt-7">
                                ...
                                <div className="flex gap-2.5">
                                    <Action
                                        ...
                                        active={favori}
                                        onClick={basculerRestaurantFavori}
                                    >
                                        <Heart
                                            size={22}
                                            fill={
                        ...
                        <section className="relative -mt-1 rounded-t-[34px] bg-jse-fond px-5 pt-6 sm:px-8">
                            <h1 className="font-against text-[2.15rem] leading-[0.98] text-jse-principal sm:text-[2.65rem]">
                                {restaurant?.nom}
                            </h1>

                            <div className="mt-2 flex items-center gap-2 font-sans text-sm text-jse-texte/60">
                                <span className="text-jse-accent">★</span>
                                <strong className="text-jse-principal">—</strong>
                                <span>Aucun avis</span>
                            </div>

                            <p className="mt-2 line-clamp-2 font-sans text-sm leading-5 text-jse-texte/65">
                            ...
                            <div className="mt-5 grid grid-cols-3 gap-2.5">
                                <div className="flex min-w-0 items-center gap-2.5 rounded-[20px] bg-white px-3 py-3.5 shadow-sm ring-1 ring-jse-texte/6 sm:px-3.5">
                                    <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-jse-accent/10 text-jse-accent">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <p className="mt-1 truncate font-sans text-[10px] text-jse-texte/50">
                                            Selon disponibilité
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="flex min-w-0 items-center gap-2.5 rounded-[20px] bg-jse-secondaire/10 px-3 py-3.5 sm:px-3.5">
                                    <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-white/55 text-jse-secondaire">
                                        <span className="size-3 rounded-full bg-jse-secondaire" />
                                    </span>
                                    <div className="min-w-0">
                                        <p className="font-sans text-xs font-bold text-jse-principal">
                                            Actif
                            ...
                            {[
                                ...
                                return (
                                    <button
                                        ...
                                        className={[
                                            "relative flex h-[62px] items-center justify-center gap-2 font-sans text-sm font-semibold",
                                            active
                                                ? "text-jse-principal"
                                                : "text-jse-texte/45",
                                        ].join(" ")}
                                    >
                                        ...
                                        {label}
                                        {active && (
                                            <span className="absolute bottom-0 left-1/2 h-1 w-[82%] -translate-x-1/2 rounded-full bg-jse-secondaire" />
                                        )}
                                    </button>
                                );
                            })}
                            ...
                            <section className="px-5 pb-36 pt-6 sm:px-8">
                                <div className="flex h-[62px] items-center gap-3 rounded-[20px] bg-white px-4 ring-1 ring-jse-texte/5">
                                    ...
                                    <input
                                        ...
                                        className="min-w-0 flex-1 bg-transparent font-sans text-sm outline-none placeholder:text-jse-texte/40"
                                    />
                                    {recherche && (
                                        <button
                                            type="button"
                                            ...
                                            className="flex size-8 items-center justify-center rounded-full bg-jse-fond"
                                        >
                                            <X size={16} />
                                        </button>
                                    )}
                                ...
                                <div className="scrollbar-none mt-4 flex gap-2.5 overflow-x-auto pb-1">
                                    <button
                                        ...
                                        className={[
                                            "shrink-0 rounded-full px-6 py-3 font-sans text-sm font-semibold",
                                            categorie === "Toutes"
                                                ? "bg-jse-principal text-white"
                                                : "bg-white text-jse-texte/65 ring-1 ring-jse-texte/5",
                                        ].join(" ")}
                                    >
                                        Tout
                                    </button>
                                ...
                                <div className="mt-5 space-y-3">
                                    {produitsFiltres.length ? (
                                        produitsFiltres.map(
                                            (produit, index) => (
                                                <article
                                                    ...
                                                    onClick={() =>
                                                        router.visit(
                                                            `/restaurants/${restaurant.id}/produits/${produit.id}`,
                                                        )
                                                    }
                                                    className="flex min-h-[126px] cursor-pointer items-center gap-3 rounded-[22px] bg-white p-2.5 shadow-sm ring-1 ring-jse-texte/5 transition hover:-translate-y-0.5 hover:shadow-md"
                                                >
                                                    <img
                                                        src={
                                                            produit.image ||
                                                    ...
                                                    <div className="min-w-0 flex-1 py-1">
                                                        ...
                                                        <p className="mt-1 line-clamp-2 font-sans text-xs leading-5 text-jse-texte/55">
                                                            {produit.description ||
                                                                "Produit disponible au menu."}
                                                        </p>
                                                        <p className="mt-2 font-sans text-lg font-extrabold text-jse-principal">
                                                            {prix(produit.prix)}{" "}
                                                            FCFA
                                                        </p>
                                        ...
                                        <div className="rounded-[24px] bg-white px-5 py-12 text-center ring-1 ring-jse-texte/5">
                                            <Search
                                                size={25}
                                                className="mx-auto text-jse-secondaire"
                                            />
                                            <h3 className="mt-3 font-against text-xl text-jse-principal">
                                                Aucun plat trouvé
                                            </h3>
                                            <p className="mt-1 font-sans text-xs text-jse-texte/45">
                        ...
                        {onglet === "avis" && (
                            <section className="px-5 pb-36 pt-10 sm:px-8">
                                <div className="rounded-[26px] bg-white px-6 py-12 text-center ring-1 ring-jse-texte/5">
                                    <MessageCircle
                                        size={30}
                                        className="mx-auto text-jse-secondaire"
                                    />
                                    <h2 className="mt-4 font-against text-2xl text-jse-principal">
                                        Avis clients
                                    </h2>
                                    <p className="mx-auto mt-2 max-w-sm font-sans text-sm leading-6 text-jse-texte/50">
                                        Aucun avis client n’est encore enregistré
                                        pour ce restaurant.
                                    </p>
                                </div>
                            </section>
                            ...
                            <section className="px-5 pb-36 pt-6 sm:px-8">
                                <div className="space-y-3">
                                    <div className="rounded-[22px] bg-white p-5 ring-1 ring-jse-texte/5">
                                        <p className="font-sans text-[10px] font-semibold uppercase tracking-[0.14em] text-jse-texte/35">
                                            À propos
                                        </p>
                                        <p className="mt-2 font-sans text-sm leading-6 text-jse-texte/70">
                                            {restaurant?.description ||
                                                "Aucune description renseignée."}
                                        </p>
                                    </div>
                                    ...
                                    <div className="rounded-[22px] bg-white p-5 ring-1 ring-jse-texte/5">
                                        <div className="flex items-start gap-3">
                                            <MapPin
                                                size={20}
                                                className="mt-0.5 shrink-0 text-jse-secondaire"
                                            />
                                            <div>
                                                <p className="font-sans text-xs font-semibold">
                                                    Adresse
                                                </p>
                                                <p className="mt-1 font-sans text-sm text-jse-texte/60">
                                                    {restaurant?.adresse ||
                                        ...
                                        <div className="mt-5 flex items-start gap-3">
                                            <Clock3
                                                size={20}
                                                className="mt-0.5 shrink-0 text-jse-accent"
                                            />
                                            <div>
                                                <p className="font-sans text-xs font-semibold">
                                                    Horaires
                                        ...
                                        <div className="mt-5 flex items-start gap-3">
                                            <Info
                                                size={20}
                                                className="mt-0.5 shrink-0 text-jse-principal"
                                            />
                                            <div>
                                                <p className="font-sans text-xs font-semibold">
                                                    Contact
                ...
                {nombre > 0 && (
                    <button
                        ...
                        className="fixed inset-x-5 bottom-5 z-40 mx-auto flex h-[72px] w-[calc(100%-40px)] max-w-[700px] items-center rounded-full bg-jse-principal px-5 text-white shadow-2xl shadow-jse-principal/25"
                    >
                        <div className="relative flex size-12 items-center justify-center rounded-full bg-white/10">
                            <ShoppingBag size={27} />
                            <span className="absolute -right-1 -top-1 flex size-6 items-center justify-center rounded-full bg-jse-accent font-sans text-[10px] font-bold">
                                {nombre > 9 ? "9+" : nombre}
                            </span>
                        </div>

                        <span className="ml-4 flex-1 text-left font-sans text-base font-bold">
                            Voir mon panier
                        </span>
                        ...
                        <ChevronRight className="ml-2" size={23} />
                    </button>
                )}
            </div>
        </main>
